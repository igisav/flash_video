...
package de.axelspringer.videoplayer.vast.parser
{
	...
	import de.axelspringer.videoplayer.vast.model.VASTVideoClick;
	import de.axelspringer.videoplayer.vast.model.VASTWrapperAd;
	...
	{		
					...
					case NODEKIND_ELEMENT:
					{
						switch (child.localName()) 
						{
							...
							case WRAPPER:
								...
								break;
							default:
								trace("[VAST] parseAdTag() - Unsupported VAST tag: " + child.localName());
								break;							
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch (child.localName()) 
						{
							...
							case SURVEY:
								...
								break;
							case ERROR:
								parseErrorTag(child, vastInlineAd);
								break;
							case IMPRESSION:
							...
							case EXTENSIONS:
								...
								break;
							default:
								trace("[VAST] parseInlineTag() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch (child.localName()) 
						{
							...
							case NON_LINEAR_ADS:
								...
								break;
							case ERROR:
								parseErrorTag(child, vastWrapperAd);
								break;
							case IMPRESSION:
							...
							case EXTENSIONS:
								...
								break;
							case VIDEO:
								vastWrapperAd.duration = parseString(child.child(DURATION));
								break;
							default:
								trace("[VAST] parseWrapperTag() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
		...
		private function parseErrorTag(xml:XML, vastAdPackage:VASTAdPackageBase):void 
		{
			var children:XMLList = xml.children();
			...
			for (var i:int = 0; i < children.length(); i++) 
			{
				...
				switch (child.nodeKind()) 
				{
					case NODEKIND_ELEMENT:
					{
						...
						{
							case URL:
								vastAdPackage.addError(parseVASTUrl(child));
								break;
						}
						break;
					}
				...
				switch (child.nodeKind()) 
				{
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							...
							default:
								trace("[VAST] parseTrackingEvents() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
		...
		private function parseURLTags(xml:XML, maxNumURLs:int=-1):Array
		{
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							case DURATION:
							{
								...
								vastInlineAd.duration = parseString(child);
								break;
							}
							case AD_ID:
							...
							case MEDIA_FILES:
								...
								break;
							default:
								trace("[VAST] parseVideo() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
				...
				switch (child.nodeKind()) 
				{
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							...
							default:
								trace("[VAST] parseMediaFiles() - Unsupported VAST tag: " + child.localName());
								break;
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							case COMPANION:
								...
								break;
							default:
								trace("[VAST] parseInlineCompanionAds() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							...
							case COMPANION_CLICK_THROUGH:
								...
								break;
							case CLICK_TRACKING:
								companionAd.clickTrackings = parseURLTags(child);
								break;
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							...
							case COMPANION_AD_TAG:
								var urls:Array = parseURLTags(child, 1);
								if (urls.length > 0)
								{
								...
								break;
							default:
								trace("[VAST] parseWrapperCompanionAds() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							...
							case NON_LINEAR_AD_TAG:
								var urls1:Array = parseURLTags(child, 1);
								if (urls1.length > 0)
								{
									vastWrapperAd.nonLinearAdTag = urls1[0];
								}
								break;
							case NON_LINEAR:
								var urls2:Array = parseURLTags( XML(child[CLICK_TRACKING]));
								if( urls2.length > 0 )
								{
									vastWrapperAd.nonLinearClickTrackings = urls2;
								}
								break;
							default:
								trace("[VAST] parseWrapperNonLinearAds() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							case NON_LINEAR:
								...
								break;
							default:
								trace("[VAST] parseInlineNonLinearAds() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
		...
		private function parseInlineNonLinearAd(xml:XML, vastInlineAd:VASTInlineAd):void 
		{
			...
			nonLinearAd.duration			= parseAttributeAsString (xml, DURATION_NON_LINEAR);
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							case NON_LINEAR_CLICK_THROUGH:
								...
								break;
							case CLICK_TRACKING:		// sevenone custom node
								nonLinearAd.clickTrackings = parseURLTags(child);
								break;
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							case EXTENSION:
								...
								break;
							default:
								trace("[VAST] parseExtensions() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
					...
					case NODEKIND_ELEMENT:
					{
						switch(child.localName()) 
						{
							...
							case CUSTOM_CLICK:
								...
								break;
							default:
								trace("[VAST] parseVideoClicks() - Unsupported VAST tag: " + child.localName());
								break;								
						}
						break;
					}
		...
		private function validate(document:VASTDocument):Boolean
		{
			...
			for each (var ad:VASTAd in document.ads)
			{
				...
				if (inlineAd != null &&
					wrapperAd != null)
				{
					trace("[VAST] Invalid VAST document:  Ad cannot have both Inline and Wrapper");
				...
				if (inlineAd != null)
				{
					...
					{
						trace("[VAST] Invalid VAST document:  InlineAd requires AdSystem, AdTitle, and Impression");
					...
					if (video != null)
					{
						...
						{
							trace("[VAST] Invalid VAST document:  Video requires Duration");
						...
						for each (var mediaFile:VASTMediaFile in video.mediaFiles)
						{
							if (mediaFile.url == null ||
								...
								mediaFile.height == 0)
							{
								trace("[VAST] Invalid VAST document:  MediaFile requires URL, plus delivery, bitrate, width and height attributes");
					...
					for each (var companionAd:VASTCompanionAd in inlineAd.companionAds)
					{
						...
						{
							trace("[VAST] Invalid VAST document:  CompanionAd requires width, height, and resourceType attributes");
					...
					for each (var nonLinearAd:VASTNonLinearAd in inlineAd.nonLinearAds)
					{
						...
						{
							trace("[VAST] Invalid VAST document:  NonLinearAd requires width, height, and resourceType attributes");
				...
				if (wrapperAd != null)
				{
					...
					{
						trace("[VAST] Invalid VAST document:  WrapperAd requires AdSystem, VASTAdTagURL, and Impression");
		...
		private function validateTrackingEvents(trackingEvents:Array):Boolean
		{
			// Validate all TrackingEvents.
			for each (var event:VASTTrackingEvent in trackingEvents)
			{
				if (event.type == null)
				{
					trace("[VAST] Invalid VAST document:  TrackingEvent requires valid type attribute");
		...
		private static const DURATION_NON_LINEAR:String			= "duration";
